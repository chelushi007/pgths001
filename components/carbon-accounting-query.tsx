  ...
  return (
    <div className="flex flex-col gap-4 p-6">
              ...
              {visible.map((r, index) => (
                <tr key={r.id} className="border-t hover:bg-muted/30">
                  ...
                  <td className="px-3 py-4">
                    <span className="inline-flex items-center gap-1.5 font-medium">{r.resource}{r.rentalSeq && <span className="rounded bg-[#e7f1ff] px-1.5 py-0.5 text-xs font-medium text-[#086de0]">第 {r.rentalSeq} 次</span>}</span>
                  </td>
                  <td className="px-3 py-4 text-muted-foreground">{r.spec}</td>
      ...
      <CarbonCertificate
        ...
        project={certificate ? { code: certificate.id, title: certificate.project, resourceName: certificate.resource, resourceSpec: certificate.spec, weight: certificate.weight, rentalSeq: certificate.rentalSeq } : null}
