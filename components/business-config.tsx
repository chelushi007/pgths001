...
const PROCUREMENT_ROLE_LABELS = ['采购方', '供应商', '平台'] as const

type Role = (typeof ROLE_LABELS)[number]
type RentalRole = (typeof RENTAL_ROLE_LABELS)[number]
type ProcurementRole = (typeof PROCUREMENT_ROLE_LABELS)[number]
...
export function BusinessConfig() {
  ...
  const [procurementWeights, setProcurementWeights] = useState<Record<ProcurementRole, number>>({ 采购方: 40, 供应商: 40, 平台: 20 })
  ...
  const procurementTotal = useMemo(() => Object.values(procurementWeights).reduce((sum, value) => sum + value, 0), [procurementWeights])
  const valid = total === 100
  const rentalValid = rentalTotal === 100
  const procurementValid = procurementTotal === 100
    ...
    <div className="min-h-full bg-[#f5f7fa] font-sans">
      ...
      <div className="p-4">
        ...
        <Card className="mt-4">
          <CardHeader className="flex flex-row items-start justify-between gap-4">
            <div>
              <CardTitle className="text-base">采购业务贡献比例</CardTitle>
              <CardDescription className="mt-1">配置采购方、供应商与平台三方的碳减排贡献比例，合计应为 100%。</CardDescription>
            </div>
            <span className={procurementValid ? 'text-sm font-medium text-primary' : 'text-sm font-medium text-destructive'}>合计 {procurementTotal}%</span>
          </CardHeader>
          <CardContent className="grid gap-3 sm:grid-cols-3">
            {PROCUREMENT_ROLE_LABELS.map((role) => (
              <label key={role} className="flex items-center justify-between gap-3 rounded-lg border bg-background px-3 py-3 text-sm">
                <span className="font-medium">{role}</span>
                <span className="flex items-center gap-1">
                  <Input type="number" min={0} max={100} value={procurementWeights[role]} aria-label={`${role}贡献比例`} onChange={(event) => setProcurementWeights((current) => ({ ...current, [role]: Math.max(0, Math.min(100, Number(event.target.value) || 0)) }))} className="h-9 w-20 text-right" />
                  <span className="text-muted-foreground">%</span>
                </span>
              </label>
            ))}
            <div className="flex justify-end sm:col-span-3"><Button disabled={!procurementValid}><Save data-icon="inline-start" />保存采购配置</Button></div>
          </CardContent>
        </Card>
      </div>
    </div>
